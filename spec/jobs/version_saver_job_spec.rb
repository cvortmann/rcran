...
describe VersionSaverJob do
  ...
  it "should save parsed version to db" do
    expect(TarGzDownloader).to receive(:new)
                               .with(package)
                               .and_return(double(:downloader, download: :tar_gz))
    expect(DescriptionFileUnpacker).to receive(:new)
                                       .with(:tar_gz)
                                       .and_return(double(:unpacker, unpack: :description_file))
    expect(DescriptionFileParser).to receive(:new)
                                     .with(:description_file)
                                     .and_return(double(:parser, parse: version_to_be_saved))
